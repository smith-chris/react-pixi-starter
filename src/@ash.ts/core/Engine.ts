...
export class Engine {
  ...
  public addEntity(entity: Entity): void {
    if (this.entityNames.has(entity.name)) {
      throw new Error(
        `The entity name ${entity.name} is already in use by another entity.`,
      )
    }
  ...
  public get entities(): Entity[] {
    ...
    for (
      let entity: Entity | null = this.entityList.head;
      entity;
      entity = entity.next
    ) {
   ...
   */
  public getNodeList<TNode extends Node>(
    nodeClass: NodeClassType<TNode>,
  ): NodeList<TNode> {
   ...
   */
  public releaseNodeList<TNode extends Node>(
    nodeClass: NodeClassType<TNode>,
  ): void {
    if (this.families.has(nodeClass)) {
   ...
   */
  public getSystem<TSystem extends System>(
    type: ClassType<TSystem>,
  ): TSystem | null {
  ...
  public get systems(): System[] {
    ...
    for (
      let system: System | null = this.systemList.head;
      system;
      system = system.next
    ) {
